...
int parsec_node_migrate_init(parsec_context_t *context)
{
    ...
    if(5 == parsec_runtime_steal_request_policy) { 
                      ...
                      void *cb_data)
{
    ...
    if (steal_request->msg.root == my_rank) /** request initiated from this node */ {
        ...
        if (2 == parsec_runtime_steal_request_policy) {
            ...
            */
            max_index = (int)(array_pos == (nb_nodes / MAX_NODES_INDEX) )   /** last victim is the same index as the nb_nodes*/
        ...
        if( !parsec_list_nolock_is_empty(ring) ) {
            ...
            while(NULL != (item = parsec_list_nolock_pop_front(ring)) ) {
                ...
                if (NULL != gpu_task) {
                    ...
                    //printf("MIG-MSG: Migrated task %s from node %d to %d  \n", tmp, get_my_rank(), steal_request->msg.root);
                    PARSEC_DEBUG_VERBOSE(10, parsec_debug_output, "MIG-MSG: Migrated task %s from node %d to %d \n", 
                        tmp, get_my_rank(), steal_request->msg.root);
...
#if defined(PARSEC_DEBUG)
    PARSEC_DEBUG_VERBOSE(10, parsec_comm_output_stream, "MIG-MSG: Migration reply send to rank %d using deps %p with pending ack %d",
                              ...
                              parsec_remote_deps_t *deps)
{
    ...
    deps->taskpool->tdm.module->incoming_message_start(deps->taskpool, deps->from, NULL /*not imp today*/, NULL /*not imp today*/,
    ...
    assert(NULL != ((parsec_remote_deps_t *)task->deps));
    ...
    for (k = 0; deps->incoming_mask >> k; k++) {
         ...
         */
        long unsigned int buf_size = sizeof(remote_dep_wire_get_t) + receiver_memory_handle_size;
...
mig_task_mapping_item_t* insert_migrated_tasks_details(parsec_task_t *task, int victim, int thief)
{
    ...
    parsec_hash_table_t* ht = task_class_hashtables->migrated_task_ht;
    assert(NULL != ht);
    ...
    if (NULL == (item = parsec_hash_table_nolock_find(ht, key)))
    {
        ...
        PARSEC_DEBUG_VERBOSE(10, parsec_comm_output_stream, "ELASTIC-MSG Rank %d: [insert_migrated_tasks_details] Task %s with key %d migrated from victim %d to thief %d tp_id %d",
            my_rank, parsec_task_snprintf(tmp, MAX_TASK_STRLEN, task), key, victim, thief, task->taskpool->taskpool_id);
...
mig_task_mapping_item_t* insert_received_tasks_details(parsec_task_t *task, int victim, int thief)
{
    ...
    mig_task_class_hashtables_t *task_class_hashtables = tp->task_class_hashtables[task->task_class->task_class_id];
    assert(NULL != task_class_hashtables);
    parsec_hash_table_t* ht = task_class_hashtables->received_task_ht;
    assert(NULL != ht);

    assert(0 <= victim && victim < get_nb_nodes());
    assert(0 <= thief && thief < get_nb_nodes());
    assert(victim != my_rank);
    ...
    if (NULL == item) {
        ...
        PARSEC_DEBUG_VERBOSE(10, parsec_comm_output_stream,"ELASTIC-MSG Rank %d: [insert_received_tasks_details] Task %s with key %d is received on from victim = %d on thief = %d",
            my_rank, tmp, key, victim, thief); 
    #endif
    }
    else {
...
{
    int nb_task_classes = tp->nb_task_classes;
    assert(NULL != tp);
...
int destroy_possible_memory_leak(parsec_taskpool_t* tp)
{
    int i = 0;
    for (i = 0; i < tp->nb_task_classes; i++) {
        assert(NULL != tp->repo_array[i]);
        parsec_hash_table_for_all(tp->repo_array[i], task_mapping_free_elt, tp->repo_array[i]);
    ...
    parsec_key_t successor_repo_key, void *param)
{
    ...
    (void)successor_repo; (void) successor_repo_key; (void)dst_rank; (void)src_rank;

    mig_task_mapping_item_t* was_received = find_received_tasks_details(newcontext);

    /** we are only interested in task that were migrated in the previous iteeration */
    ...
    if(was_received) {
        ...
        //    dep->flow->flow_index,
        //    deps->from, deps->root);
        PARSEC_DEBUG_VERBOSE(10, parsec_comm_output_stream, "ELASTIC-MSG Rank %d: [mig_dep_mpi_retrieve_datatype] Task %s with (with predecessor %s) migrated from victim %d to thief %d for flow %d will be received(deps from %d root %d)",
            get_my_rank(), 
            tmp1, tmp2, 
            was_received->victim, was_received->thief, 
            dep->flow->flow_index,
    ...
    deps->outgoing_mask++; /** temp storage: count the received tasks */
                            ...
                            remote_dep_datakey_t complete_mask)
{
    ...
    PARSEC_DEBUG_VERBOSE(10, parsec_debug_output, "ELASTIC-MSG Rank %d: [mig_direct_release_incoming] Release direct deps from Task %s (depd from node %d taskpool_d %d)",
            my_rank, parsec_task_snprintf(tmp1, MAX_TASK_STRLEN, &task), origin->from, origin->taskpool->taskpool_id);
#endif
                                 ...
                                 void *param)
{
    ...
    PARSEC_DEBUG_VERBOSE(10, parsec_debug_output, "ELASTIC-MSG Rank %d: [parsec_gather_direct_collective_pattern] Task %s with (with predecessor %s) migrated from victim %d to thief %d for flow %d is propogated (deps from %d root %d)",
                            get_my_rank(), 
                            tmp1, tmp2, 
                            was_migrated->victim, was_migrated->thief, 
                            dep->flow->flow_index,
...
int create_task_class_hashtables(parsec_context_t *context) 
{
    ...
    for(i = 0; i < NB_TASK_CLASS_HTS; i++ ) {
        ...
        task_class_hashtables[i]->migrated_task_ht = PARSEC_OBJ_NEW(parsec_hash_table_t);
        parsec_hash_table_init(task_class_hashtables[i]->migrated_task_ht, offsetof(mig_task_mapping_item_t, ht_item), 16, 
            node_task_mapping_table_generic_key_fn, NULL); 

    }

    assert(NULL != task_class_hashtables);
    context->task_class_hashtables = task_class_hashtables;
...
int destroy_task_class_hashtables(parsec_context_t *context) 
{
    ...
    for(i = 0; i < NB_TASK_CLASS_HTS; i++ ) {
        ...
        PARSEC_OBJ_RELEASE(task_class_hashtables[i]->received_task_ht);
        task_class_hashtables[i]->received_task_ht = NULL; 


        parsec_hash_table_for_all(task_class_hashtables[i]->migrated_task_ht, task_mapping_free_elt, 
            task_class_hashtables[i]->migrated_task_ht);
        parsec_hash_table_fini(task_class_hashtables[i]->migrated_task_ht);
        PARSEC_OBJ_RELEASE(task_class_hashtables[i]->migrated_task_ht);
        task_class_hashtables[i]->migrated_task_ht = NULL; 
