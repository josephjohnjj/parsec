...
static void jdf_generate_structure(jdf_t *jdf)
{
    ...
    if(nbfunctions != 0 ) {
        ...
        coutput("  data_repo_t* repositories_mig[%d];\n", nbfunctions);
    }
    ...
    coutput("/* Data Repositories */\n");
    {
        ...
        for( f = jdf->functions; NULL != f; f = f->next ) {
             ...
             * No longer only when if( !(f->flags & JDF_FUNCTION_FLAG_NO_SUCCESSORS) )
             */
            coutput("#define %s_repo_mig (__parsec_tp->repositories_mig[%d])\n",
                    f->fname, f->task_class_id);
        }
    }
...
static void jdf_generate_internal_init(const jdf_t *jdf, const jdf_function_entry_t *f, const char *fname)
{
    ...
    coutput("  __parsec_tp->repositories_mig[%d] = data_repo_create_nothreadsafe(%s, %s, (parsec_taskpool_t*)__parsec_tp, %d);\n",
            f->task_class_id, need_to_count_tasks ? "nb_tasks" : "PARSEC_DEFAULT_DATAREPO_HASH_LENGTH",
            jdf_property_get_string(f->properties, JDF_PROP_UD_HASH_STRUCT_NAME, NULL),
            idx );
...
static void jdf_generate_destructor( const jdf_t *jdf )
{
    ...
    for( f = jdf->functions; NULL != f; f = f->next ) {
        ...
        coutput("   data_repo_destroy_nothreadsafe(__parsec_tp->repositories_direct[%d]);  /* %s */\n",
                f->task_class_id, f->fname);
        coutput("   data_repo_destroy_nothreadsafe(__parsec_tp->repositories_mig[%d]);  /* %s */\n",
                f->task_class_id, f->fname);
    }
...
static void jdf_generate_constructor( const jdf_t* jdf )
{
    ...
    coutput("  __parsec_tp->super.super.repo_array_mig = %s;\n",
            (NULL != jdf->functions) ? "__parsec_tp->repositories_mig" : "NULL");
                                                     ...
                                                     jdf_dep_flags_t flow_type)
{
    ...
    coutput("#if defined(DISTRIBUTED)\n"
            ...
            "if(remote_dep_mpi_retrieve_datatype == ontask || parsec_gather_collective_pattern == ontask || mig_dep_mpi_retrieve_datatype == ontask || parsec_gather_direct_collective_pattern == ontask) \n"
